...
TOOL: store_medical_report
  Parameters: report_type (str), key_findings (list), lab_values (dict),
              health_summary (str), severity_level (str), recommendations (list)
  Returns: Confirmation with report_id and storage status
  Use AFTER Gemma 4 has analyzed a medical report image — extracts and stores results to DB
  NOTE: Image analysis is done natively by Gemma 4 via the multimodal prompt.
        This tool only STORES the extracted results.
...
┌─────────────────────────────────────────────────────────────────────────────┐
│  MEDICATION IMAGE INTENT — handled by Gemma 4 natively:                      │
│  1. The model already sees the image in the conversation context             │
│  2. Extract medication info directly from the model's analysis              │
│  3. Package: medication_name + active_ingredient + dosage                   │
│  4. Do NOT call any tool — just format the model's analysis                 │
└─────────────────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────────────┐
│  MEDICAL REPORT IMAGE INTENT — Gemma 4 analysis + storage:                   │
│  1. The model already sees the report image in the conversation context     │
│  2. Extract: report_type, key_findings, lab_values, health_summary,        │
│     severity_level, recommendations                                         │
│  3. Call store_medical_report(...) to save results to database              │
│  4. Package: all extracted data + safety disclaimers                         │
│  5. If severity_level = CRITICAL → flag as urgent, emphasize findings      │
