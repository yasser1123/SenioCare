...
| Phase | State | Notes |
|---|---|---|
| 0 Baseline & hygiene | done | plus 0.5 DB pool/deadlines (F-01) |
...
| 0.5 | `fix(db): connection pool + client-side deadlines` | **Added during Phase 0, not in the original plan.** The suite stalled at 0% CPU on two consecutive runs. Root cause: Neon's pooler hostname resolves to three IPs, one blackholes TCP from this network, and `psycopg2.connect` had no `connect_timeout`; separately, queries on established connections occasionally never returned. `database.py` now pools connections (one per process instead of one per tool call), bounds connects with libpq's `connect_timeout` and queries with a psycopg2 wait-callback deadline. Suite: 108 s with random hangs → 56 s, deterministic | Blocks every later phase: the eval baseline runs cannot hang |
...
| # | Finding | Fix | Test |
|---|---|---|---|
| 4.1 | **C-04** tool guards persist across turns | Turn-aware guard (`seniocare/tools/_guards.py`): the flag stores the turn number it was set in and only blocks when it equals `conversation_turn_count`. *Not* a `temp:` prefix: ADK never merges `temp:` keys into `session.state` (`BaseSessionService._update_session_state`), so a temp flag would be invisible to the next tool call inside the same turn too | `tests/test_tool_guards.py` + the `mt-guard-*` scenarios |
...
| 4.16 | **R-01** event-loop blocking | DB tool functions become `async def` wrapping the psycopg2 work in `asyncio.to_thread`; `requests` → `httpx.AsyncClient` (already a dependency) in `web_search.py`. *(Connection-per-call and unbounded waits were already removed in 0.5; what remains is moving the work off the event loop.)* | Existing unit tests updated; latency visible in Phase 2 metrics |
